...
large_scale={'size':'large','x_size':30,'y_size':16,'mine':99,'start_x':223,'start_y':145,'end_x':1696,'end_y':931}
for x in range(30):
    for y in range(16):
        large_scale[(x,y)]=[248+x*49,170+y*49]


middle_scale={'size':'middle','x_size':16,'y_size':16,'mine':40,'start_x':567,'start_y':145,'end_x':1352,'end_y':930}
for x in range(16):
    for y in range(16):
    ...
    for i in range(game_scale['y_size']):
        for j in range(game_scale['x_size']):
            if matrix[i][j] == -1 or (matrix[i][j] < 1 and matrix[i][j] > 0):
                roi = img_gray[game_scale['start_y'] + square['length'] * i - 1 :game_scale['start_y'] + square['length'] * (i + 1) + 1,
                                game_scale['start_x'] + square['width'] * j - 1 :game_scale['start_x'] + square['width'] * (j + 1) + 1 ]
